...
当前仓库阶段：**前后端可联调的纯手动输入 MVP 已成型，并已接入候选反推软评分、真实样本校正后的 PVP 面板公式、候选生成默认性能收缩、普通攻击伤害、P0 状态伤害、星陨计算、阶段 C 自动结算、阶段 D 技能效果操作执行器、RuleResolver 雏形和阶段 E 的 ModifierResolver/ResponseResolver 最小闭环；正在继续补齐候选评分写入性能、天气/状态 modifier、完整应对/防御结算和事件重放**。
...
- 候选生成：默认使用 `standard` 主流极速空间，要求恰好 3 个维度投入资质、正面性格只考虑生命/物攻/魔攻/速度、正面资质固定 10、另外两维只取 9/10、负面性格不考虑减少生命/物防/魔防、负面性格维度必定为 0、物攻资质和魔攻资质互斥；单只敌方默认候选约 188。另保留 `wide` 约 576、`balanced` 约 3888 和 `full` 46320 分层模式用于匹配失败后的兜底扩展。
...
- 前端联动：战斗工作台伤害录入可同步提交 observation，技能使用可触发已入库的结构化操作，候选面板可在 Top5 性格与个体资质组合中选择一个组合并展示估计六维面板；工作台已接入结束回合按钮和最近回合结算摘要，事件日志可选择事件、作废/修正/重放占位并查看快照详情。
...
- 候选配置硬排除尚未开启；当前以软评分、置信度和 evidence 记录为主，避免未验证公式污染候选。后续硬排除应只在技能确认、快照完整、公式上下文无关键 unknown 且容差明确时启用。
...
1. 继续优化候选评分写入：伤害观察按面板组批量更新 `match_score`，减少逐候选 ORM 读写和 evidence JSON 追加。
2. 补完整候选 evidence 解释链，先把事件级公式上下文和分组评分摘要展示清楚。
3. 在公式、快照、技能和应对上下文完整时，增加“受保护硬排除”开关与证据记录；默认仍保持关闭。
4. 继续扩展 `ModifierResolver`：接入天气/状态 modifier、更多快照状态修正与特殊公式。
5. 补完整应对/防御独立结算层，并实现事件重放重算。
