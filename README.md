...
1. **数据库结构**：只由 Alembic 负责。手动执行 `python -m alembic upgrade head` 仍然可用；后端启动时也会先自动执行一次 `upgrade head`，因此新电脑或空 `data/app.db` 不会因为缺表导致启动失败。
2. **核心性格与默认技能规则**：数据库结构就绪后，后端启动时会自动幂等自检查并写入/修正 30 种性格，以及通用默认技能“聚能”。
