...
@router.get("/{battle_id}/{elf_id}/detail", response_model=CandidateDetailOut)
def get_candidate_detail(
    battle_id: str,
    elf_id: str,
    include_excluded: bool = Query(default=False, description="是否包含已排除候选"),
    speed_bucket_size: int = Query(default=5, ge=1, le=50, description="速度分桶宽度"),
    db: Session = Depends(get_db),
) -> CandidateDetailOut:
    """获取候选配置详情页聚合数据。"""
    try:
        BattleService(db).require_battle(battle_id)
        return CandidateService(db).get_detail(
            battle_id,
            elf_id,
...
@router.get("/{battle_id}/{elf_id}/evidence", response_model=CandidateEvidenceOut)
