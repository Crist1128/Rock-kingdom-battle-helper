...
class PlayerElfBuildOut(BaseModel):
    ...
    skill_ids 来自 player_elf_build_skill 表，服务层负责合并。为了让前端在配置
    列表和准备阶段下拉框中稳定显示中文精灵名，本输出同时冗余返回精灵展示
    字段；这些字段只读，不改变数据库结构。
    """
...
class PlayerElfBuildUpdate(PlayerElfBuildCreate):
    """
    更新己方精灵配置请求。

    当前采用“整单替换”策略：前端提交编辑页上的完整配置，后端重新计算面板
    属性并按 skill_ids 顺序重建技能槽。这样可以避免局部 PATCH 时出现技能槽
    顺序和面板缓存不一致的问题。
    """


class PlayerElfBuildSkillReplace(BaseModel):
    """替换己方配置技能槽请求，skill_ids 的顺序就是最终槽位顺序。"""
