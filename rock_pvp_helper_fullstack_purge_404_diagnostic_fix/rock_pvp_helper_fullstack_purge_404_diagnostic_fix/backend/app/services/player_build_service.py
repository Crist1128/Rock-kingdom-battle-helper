...
from app.schemas.player_build import PlayerElfBuildCreate, PlayerElfBuildOut, PlayerElfBuildUpdate
...
class PlayerElfBuildService:
    ...
    def update_build(self, build_id: str, payload: PlayerElfBuildUpdate) -> PlayerElfBuildOut:
        """
        更新己方精灵配置，并重新计算面板属性与技能槽顺序。

        设计说明：
        - 这里使用完整替换而不是局部更新，因为面板属性由 elf_id、nature_id、
          individual_talent_distribution 三者共同决定；只更新其中一项时也必须
          重新计算 final_stats_json。
        - skill_ids 的数组顺序就是技能槽位顺序。前端拖拽排序后只要提交新的
          skill_ids 即可，无需单独维护 slot_index。
        """
        ...
        if payload.is_default:
            self._clear_default_builds(payload.elf_id, exclude_build_id=build_id)

        build.elf_id = payload.elf_id
        build.nature_id = payload.nature_id
        build.build_name = payload.build_name
        build.individual_talent_distribution_json = dumps_json(individual)
        build.final_stats_json = dumps_json(final_stats)
        build.is_default = payload.is_default
        build.notes = payload.notes

        self._replace_skill_rows(build_id, payload.skill_ids)
        self.db.commit()
    ...
    def delete_build(self, build_id: str) -> None:
        ...
        不物理删除配置主体，避免历史战斗中保存的 build_id 断链。普通列表
        接口会过滤 deleted_at，因此前端删除后不会再展示该配置。
        """
        build = self.db.get(PlayerElfBuild, build_id)
        if build is None or build.deleted_at is not None:
            raise LookupError(f"己方配置不存在：{build_id}")
    ...
    def _replace_skill_rows(self, build_id: str, skill_ids: list[str]) -> None:
        """按 skill_ids 的顺序重建技能槽行。"""
    ...
    def _clear_default_builds(self, elf_id: str, exclude_build_id: str | None = None) -> None:
        """同一精灵只保留一个默认配置，可排除当前正在更新的配置。"""
        stmt = select(PlayerElfBuild).where(
        ...
        if exclude_build_id is not None:
            stmt = stmt.where(PlayerElfBuild.build_id != exclude_build_id)
        builds = self.db.scalars(stmt).all()
        for item in builds:
            item.is_default = False

    def _to_out(self, build: PlayerElfBuild) -> PlayerElfBuildOut:
        """将配置主体、精灵展示字段与技能槽合并为 API 输出结构。

        前端配置列表和准备阶段下拉框不能依赖一次性拉取全部精灵列表，因为
        全量规则库可能超过分页上限。这里直接把当前配置所属精灵的中文名、
        头像和系别冗余返回，既不改数据库，也能保证列表稳定显示中文信息。
        """
        ...
        elf = self.db.get(ElfDefinition, build.elf_id)
    ...
    def replace_build_skills(self, build_id: str, skill_ids: list[str]) -> PlayerElfBuildOut:
        ...
        self._replace_skill_rows(build_id, skill_ids)
        self.db.commit()
        return self.get_build(build_id)
