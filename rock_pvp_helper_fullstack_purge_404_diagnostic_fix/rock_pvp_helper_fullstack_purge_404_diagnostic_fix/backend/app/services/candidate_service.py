...
class CandidateService:
    ...
    def get_detail(
        ...
        *,
        ...
        speed_bucket_size: int = 5,
    ) -> CandidateDetailOut:
        """
        获取候选配置详情页需要的聚合数据。

        聚合范围默认只包含未排除候选。当前伤害公式未实现，因此一般情况下
        active_count 会等于 total_count；等公式接入后，该接口会自然展示过滤后
        候选池的分布变化。
        """
        summary = self.summarize(battle_id, elf_id)
        ...
        total = len(rows)
        if total == 0:
            return CandidateDetailOut(summary=summary)

        return CandidateDetailOut(
            summary=summary,
            speed_buckets=self._build_speed_buckets(rows, speed_bucket_size),
    ...
    def _load_candidates_for_detail(
        ...
        *,
        ...
        单只精灵第一阶段约 4.6 万候选，Python 侧聚合可以接受，并能更方便解析
        individual_talent_distribution_json。后续如果候选规模继续扩大，可把速度和
        性格分布下推到 SQL 聚合。
        """
        stmt = select(BuildCandidate).where(
            BuildCandidate.battle_id == battle_id,
    ...
    @staticmethod
    def _build_speed_buckets(
        rows: list[BuildCandidate],
        bucket_size: int,
    ) -> list[CandidateSpeedBucketItem]:
        """把候选速度按固定宽度分桶，便于前端展示速度区间分布。"""
        bucket_size = max(bucket_size, 1)
        total = len(rows)
        buckets: dict[int, int] = {}
        for row in rows:
            start = (row.final_speed // bucket_size) * bucket_size
            buckets[start] = buckets.get(start, 0) + 1
        return [
            CandidateSpeedBucketItem(
                min_speed=start,
                max_speed=start + bucket_size - 1,
                count=count,
                ratio=count / total,
            )
    ...
    @staticmethod
    def _build_nature_distribution(
        rows: list[BuildCandidate],
    ) -> list[CandidateNatureDistributionItem]:
        """统计候选池中各性格出现次数。"""
        total = len(rows)
        counts: dict[str, int] = {}
        for row in rows:
            counts[row.nature_id] = counts.get(row.nature_id, 0) + 1
        return [
            CandidateNatureDistributionItem(nature_id=nature_id, count=count, ratio=count / total)
            for nature_id, count in sorted(counts.items(), key=lambda item: (-item[1], item[0]))
        ]

    @staticmethod
    def _build_talent_distribution(
        rows: list[BuildCandidate],
    ) -> list[CandidateTalentDistributionItem]:
        """统计六维个体资质的取值分布。"""
        stat_keys = [key.value for key in STAT_KEYS]
        counts: dict[str, dict[str, int]] = {stat_key: {} for stat_key in stat_keys}
        non_zero_counts: dict[str, int] = {stat_key: 0 for stat_key in stat_keys}
        for row in rows:
            data = loads_json(row.individual_talent_distribution_json, {})
            if not isinstance(data, dict):
                data = {}
            for stat_key in stat_keys:
                value = int(data.get(stat_key, 0) or 0)
                value_key = str(value)
                counts[stat_key][value_key] = counts[stat_key].get(value_key, 0) + 1
                if value > 0:
                    non_zero_counts[stat_key] += 1
        total = len(rows)
        return [
            CandidateTalentDistributionItem(
                stat_key=stat_key,
                non_zero_count=non_zero_counts[stat_key],
                zero_count=total - non_zero_counts[stat_key],
                value_counts=dict(sorted(counts[stat_key].items(), key=lambda item: int(item[0]))),
            )
            for stat_key in stat_keys
        ]

    @staticmethod
    def _build_pattern_distribution(
        rows: list[BuildCandidate],
    ) -> list[CandidatePatternDistributionItem]:
        """统计个体资质存在维度组合，默认只返回最多的前 20 项。"""
        total = len(rows)
        counts: dict[str, int] = {}
        stat_keys = [key.value for key in STAT_KEYS]
        for row in rows:
            ...
            active_keys = [
                stat_key
                for stat_key in stat_keys
                if int(data.get(stat_key, 0) or 0) > 0
            ]
            pattern = "+".join(active_keys) if active_keys else "none"
