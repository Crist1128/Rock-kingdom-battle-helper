## 2026-07-21 星陨伤害计算器交互修正

1. 星陨计算器布局调整为“我方精灵 → 敌方精灵 → 星陨层数与结果”，保持工具包内的深空科技风与双主题语义色。
2. 我方/敌方精灵卡支持搜索后自选性格与六维资质，也可从最近战斗导入并切换为面板值计算。
3. 星陨层数去掉圆形层数控件，仅保留滑条和精确输入；满足条件后自动计算，拖动层数时伤害环和数值同步刷新。
4. 触发与参数区补齐普通伤害计算器常用输入：触发技能、连击、威力覆盖、本系/克制/天气/能力/减伤，以及层级快捷修正。
5. 最近战斗快捷导入改为“我方队伍 / 敌方队伍”两个可展开列表，导入目标更清晰。
6. 顶部说明文案收起，标题旁新增自动计算开关；触发与参数区下移为整行输入区，星陨层数与伤害结果上下相邻。

---

# 本次前端修改说明

## 2026-07-21 星陨伤害计算器首版

1. **侧边栏工具包分组**：将“伤害计算器”和“星陨伤害计算器”归类到可展开的“工具包”，折叠侧栏下保留图标与 tooltip。
2. **新增星陨计算页面**：采用“我方左侧 / 参数中置 / 敌方右侧”的对战布局，支持选择攻防精灵、导入最近战斗面板、选择触发技能、填写星陨层数、连击、威力覆盖、克制覆盖、减伤与实战总伤害。
3. **结果解释链**：中间结果环展示准确总伤害和扣血百分比，并补充技能伤害、星陨伤害、剩余 HP、击杀判断；下方保留技能与星陨公式关键输入，延续当前深空科技风和深浅双主题。
