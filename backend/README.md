...
当前版本重点完成 **手动输入 MVP 后端闭环 + 候选反推软评分初步闭环**：己方配置、战斗阵容、敌方候选、统一状态实例、伤害事件记录、状态快照、数据管线管理接口、Observation API、普通攻击伤害计算和 RuleResolver 雏形。
...
9. 每个关键事件生成状态快照。
10. 伤害录入可进一步提交 observation，由后端进行候选软评分。
11. 公式或上下文存在未知因素时，只记录 unknown/evidence，不硬排除候选。

## 重要边界

当前版本不会自动识图，也不会自动推荐出招。伤害计算已具备普通攻击最小公式和规则解析雏形，但完整真实伤害体系仍未全部实现；状态伤害、星陨、复杂技能分支、天气/状态 modifier 等仍需继续接入。候选推断默认只软评分，不因未验证公式或未知因素硬排除。
...
当前后端已经完成手动输入 MVP 的主要闭环：己方配置、战斗流程、事件日志、状态实例、快照、候选生成、rocom 数据管线和管理接口均已具备。后续又补齐了 Observation API、候选软评分骨架、普通攻击最小伤害公式和 RuleResolver 雏形。
...
3. 实现状态自动结算与星陨等特殊伤害。
4. 完善候选 evidence 查询与前端证据链展示。
5. 在样例验证充分后，再逐步开启候选硬排除。
6. 实现事件重放重算。
