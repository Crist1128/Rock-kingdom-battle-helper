...
> 当前状态：本文是旧进展快照。项目主线已从“观测驱动候选软评分”切换为 `EnemyPanelEstimate` 实时面板反推；旧候选空间和候选硬排除不再作为业务目标。

## 当前后端阶段

后端已经完成纯手动输入 MVP 的主要闭环，并已进入“实时面板估计 + 普通攻击伤害计算 + RuleResolver 雏形”阶段。当前重点是补齐正式状态定义、技能规则分支、状态/天气反推、实时估计证据链和事件重放；旧候选硬排除路线已废弃。
...
4. 实现状态自动结算与星陨等特殊伤害。
5. 完善实时估计 evidence 聚合查询和前端解释链展示。
6. 完善默认配置保存失败时的约束解释。
7. 接入状态/天气/速度反推，关键未知因素只写 unknown factors。
8. 实现真实事件重放服务。
