...
class Settings(BaseSettings):
    ...
    # 数据库配置
    database_url: str = _sqlite_url_from_path(
        DEFAULT_DATA_DIR / "app.db"
    )  # 默认写入项目根目录 data/app.db

    # 数据更新管理配置
    admin_update_token: str | None = None  # 配置后，数据更新接口要求 X-Admin-Token
    rocom_data_dir: str = str(
        DEFAULT_DATA_DIR / "rocom"
    )  # 默认写入项目根目录 data/rocom，不应提交 Git
