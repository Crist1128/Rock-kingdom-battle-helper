...
- 2026-07-19 基于《Codex_敌方精灵头像视觉识别复现说明.md》新增透明模板库头像识别原型：支持截图读取、透明 PNG/ZIP 素材、固定布局裁槽、Alpha 遮罩粗排、Lab 颜色复排、Top2 输出与调试产物导出；已直接接入准备页识别接口作为默认实现。
- 2026-07-19 继续优化透明模板库头像识别：识别器改为进程级缓存，粗筛与精排继续收窄候选集，新增 `comparison/slot_{n}_detail.png` 和 `comparison/slot_{n}_top2.png` 详细抠像对比图，方便人工复核识别效果。
- 2026-07-19 准备页识别接口新增 `include_debug` 参数，默认把整图框选、总览图、每槽裁剪、Top1 抠像/对齐/差异和 Top2 对比图保存到 `data/rocom/recognition/debug_runs/`，响应返回可直接访问的 `debug_artifacts` URL；前端识别结果区已直接展示这些调试图。旧调试目录会由后续请求按 24 小时 TTL 和最多 50 次运行上限清理。
...
- 已实现普通攻击最小伤害计算、基础状态伤害计算、星陨伤害计算、伤害观测匹配、`RuleResolver` 雏形、`ModifierResolver` 与 `ResponseResolver` 最小闭环。
...
- 设置页已新增项目静态规则同步检查入口：可列出 seed 中待同步的状态定义与已完备技能分支，并由用户选择 dry-run 或确认写库；后端普通启动仍不会自动批量写入静态规则。
...
- 已接入 rocom 数据更新管理入口和归档战斗清理入口；设置页已补充“新用户 / 空库一键初始化”，可检查必要数据、从本地 cleaned 包 dry-run/提交初始化，并复用后台任务进度条。
...
- 状态定义已可通过项目 seed 和 effect importer 写入；当前工作库已导入基础状态定义，并通过技能人工审阅、机制补齐和确认印记/蓄力时序机制额外补入属性/速度/技能层数/印记/行动修正状态。
...
- 完整真实伤害体系尚未全部实现：当前已有普通攻击最小公式、基础状态伤害、星陨、阶段 C 自动结算、阶段 D 技能效果操作和阶段 E 最小减伤/应对解析；复杂公式分支、天气/状态 modifier、完整应对/防御独立结算仍需补齐。
...
3. 先补状态定义：灼烧、中毒、寄生、冻结、棘刺印记、中毒印记、星陨印记，以及必要天气。
...
- 2026-07-21 新增星陨伤害计算器首版：后端提供只读 `POST /api/v1/damage-calculator/starfall-combo`，复用本地技能、精灵面板、属性克制与现有伤害公式链，返回触发技能伤害、星陨印记伤害、总伤害、扣血比例、剩余 HP 与解释链；前端侧边栏新增“工具包”分组，将独立伤害计算器与星陨伤害计算器归类展示。该功能不迁移外部项目数据或 UI，不联网抓取 BWIKI，也不写入战斗事件/evidence。
