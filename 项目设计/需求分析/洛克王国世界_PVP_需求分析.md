# 洛克王国世界_PVP_需求分析_v0.3.3.1

# 洛克王国世界 PVP 战斗信息获取与敌方配置推算系统需求分析 v0.3.3.1
...
- 连击数 +3、连击数 -4、行动时连击数 +100%、连击数固定为2。
- 吸血 +100%。
...
~~每个队伍最多同时存在 1 个增益印记和 1 个减益印记。~~（里拉鳐可以做到）
```
...
| 减益印记 | 中毒印记 | 疫病吐息、蚀刻（裘卡） | 敌方获得 1 层 |
| 减益印记 | 星陨印记 | 二律背反、心灵洞悉、星轨裂变、星链、超新星馈赠、超维投射 | 敌方获得指定层数；作为减益印记占用减益印记槽，不通过切换清除 |
...
$$
\text{属性值} = （\text{基础值} + \text{等级} \times \text{资质值}）× \text{性格修正} + \text{成长值}
$$
...
| element_types | array | 是 | 数据库录入 | 精灵系别。可能存在单系或双系，因此使用数组。 |
...
| learnable_skill_ids | array | 是 | 数据库录入 | 该精灵所有可能技能 ID。敌方技能未知时用于枚举“技能 + 配置”联合候选。 |
| common_skill_sets | array<array> | 否 | 数据库录入 / 统计 | 常见技能组。用于候选排序，不应直接排除不常见技能。 |
| common_natures | array | 否 | 数据库录入 / 统计 | 常见性格分布。可作为候选初始权重。 |
| common_individual_talent_patterns | array | 否 | 数据库录入 / 统计 | 常见个体资质维度与数值组合。可作为候选初始权重。 |
| forms | array | 否 | 数据库录入 | 形态、皮肤、进化、同名不同形态。若不同形态种族资质不同，应拆成不同 `pet_id`。 |
| recognition_templates | array | 否 | 图像识别维护 | 头像区域、名称区域、颜色特征等识别模板。由于精灵没有别名，识别纠错不依赖别名表。 |
...
| alias_names | array | 否 | 数据库录入 / 手动维护 | 别名、OCR 常见误识别名。 |
...
| tags | array | 否 | 数据库录入 / 结构化整理 | 快速粗分类，用于检索和 UI。示例：`quick`、`charge`、`transmission`、`defense_response`、`multi_hit`、`mark_related`、`weather_setter`。 |
...
| effect_script | array | 否 | 数据库录入 | 技能使用后的效果操作列表。状态技能和附加状态攻击技能都需要。详见 `EffectOperation`。 |
...
| per_hit_effect_script | array | 否 | 数据库录入 | 每段命中后触发的状态脚本。例如“每次连击给敌方 1 层星陨”。MVP 可先不实现逐段触发，只做技能级近似。 |
...
| stat_modifiers | array | 否 | 数据库录入 | 属性修正列表。示例：`[{stat:"speed", value_type:"flat", value:-90}]` 或 `{stat:"physical_attack", value_type:"percent", value:100}`。 |
| damage_modifiers | array | 否 | 数据库录入 | 增伤、减伤、吸血、反伤、最终伤害倍率等。注意防御技能减伤可作为 `defense_skill_state`。 |
| skill_cost_modifiers | array | 否 | 数据库录入 | 技能能耗修正，例如全技能能耗+3、攻击技能能耗+6、下次技能能耗-6。 |
| skill_power_modifiers | array | 否 | 数据库录入 | 技能威力修正，例如光系技能威力+40%、全技能威力+40、下一次攻击技能威力翻倍。 |
| action_modifiers | array | 否 | 数据库录入 | 行动相关修正，例如先手+1、先手-1、迅捷、蓄力、打断、眩晕、无法更换、脱离、返场。 |
| trigger_rules | array | 否 | 数据库录入 | 触发规则，例如回合结束触发、受击后触发、切换入场触发、应对防御触发。 |
| settlement_rules | array | 否 | 数据库录入 | 结算规则，例如灼烧伤害、中毒伤害、寄生回复、生命交换、能量偷取。 |
...
| effect_hooks | array | 否 | 数据库录入 | 印记造成的具体影响，例如受击增伤、行动结算、技能特殊联动。没有明确效果时也要记录层数。 |
...
| effect_hooks | array | 否 | 数据库录入 | 天气对技能威力、能耗、伤害、状态结算的影响。 |
...
| possible_builds | array | 是 | 敌方建档模块 | 当前仍可能的候选配置集合。每次伤害事件后更新。 |
| confirmed_stats | object | 否 | 系统推算 / 手动确认 | 已基本确认的面板属性。未确认时为空或只填已确认维度。 |
| possible_skills | array | 是 | 精灵数据库 / 推算模块 | 仍可能存在的技能 ID 集合。准备阶段来自技能池，战斗中逐步缩小。 |
| confirmed_skills | array | 否 | 技能识别 / 手动输入 | 已确认敌方使用过或明确携带的技能。 |
...
| battle_effects | array | 否 | 状态管理模块 | 与该精灵直接相关的统一状态实例。计算层以此为主。 |
| active_normal_buffs | array | 否 | 派生 | 普通增益展示分组，由 `battle_effects` 按 category 派生。 |
| active_normal_debuffs | array | 否 | 派生 | 普通减益展示分组，切换时通常清除。 |
| abnormal_stack_states | array | 否 | 派生 | 异常 / 层数状态展示分组，如冻结、中毒、灼烧、萌化等。 |
| skill_slot_effects | array | 否 | 状态管理模块 | 技能槽修正，例如能耗变化、威力变化、冷却、技能变形。 |
| action_rule_effects | array | 否 | 派生 | 迅捷、蓄力、打断、眩晕、无法更换、脱离、返场等行动规则效果。 |
...
| evidence_log | array | 否 | 推算模块 / 事件日志 | 支撑该状态的证据摘要或事件 ID 列表。 |
...
| possible_skills | array | 是 | 技能池 / 推算模块 | 在该候选下仍可能的技能。技能未知时与配置联合枚举。 |
...
| matched_event_ids | array | 否 | 推算模块 | 支持该候选的事件 ID。 |
| mismatched_event_ids | array | 否 | 推算模块 | 与该候选冲突的事件 ID。 |
...
| hit_results | array | 否 | 伤害计算模块 | 多段技能逐段理论结果。默认不要求生成；MVP 可为空，只展示总伤害区间。 |
...
| hit_details | array | 否 | 图像识别 / 手动输入 | 多段逐段详情。MVP 默认不记录，可为空。 |
...
| triggered_effects | array | 否 | 状态管理模块 | 当前段触发的附加效果，例如每段叠星陨。MVP 可不记录。 |
...
| attacker_effects | array | 是 | 状态管理 | 攻击方精灵状态快照。 |
| defender_effects | array | 是 | 状态管理 | 防御方精灵状态快照。 |
| attacker_side_marks | SideMarkState | 是 | 状态管理 | 攻击方队伍印记槽快照。 |
| defender_side_marks | SideMarkState | 是 | 状态管理 | 防御方队伍印记槽快照。 |
| battlefield_effects | array | 否 | 状态管理 | 天气和其他全战场效果。 |
| attacker_skill_slot_effects | array | 否 | 状态管理 | 攻击方技能槽效果。 |
| defender_skill_slot_effects | array | 否 | 状态管理 | 防御方技能槽效果。 |
| turn_effects | array | 否 | 状态管理 | 只在当前回合有效的临时效果。 |
