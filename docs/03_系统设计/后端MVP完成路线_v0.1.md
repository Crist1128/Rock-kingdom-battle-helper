...
> 当前落地状态：本文是旧后端 MVP 路线记录。旧候选软评分/硬排除路线已废弃；当前后端主线改为实时面板估计、属性约束、evidence、默认配置校验和事件流重算。涉及 `BuildCandidate`、候选 Top、候选硬排除的内容仅作为历史背景。
...
3. 系统能把伤害观测、状态触发、技能出现、速度先后手写入实时估计约束和 evidence。
4. 实时估计结果能给出解释链，说明为什么收窄属性、记录 unknown 或产生冲突。
5. 事件修正后可以重放重算，避免历史解释读取当前状态。
6. 所有静态规则导入都有 dry-run、commit 和可审阅摘要。

旧候选硬排除不再作为 MVP 目标。MVP 阶段默认坚持实时约束和 evidence；只有公式、状态快照和技能分支完整可信时，才允许收窄对应属性范围。
