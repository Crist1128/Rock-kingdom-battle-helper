...
> 当前落地状态：本文是早期开发规格基线。旧 `BuildCandidate` 候选空间、候选过滤和候选硬排除设计已废弃；当前新开发以 `enemy_panel_estimate`、`enemy_panel_estimate_evidence`、`EstimateService` 和按需内存展开为准。旧候选字段章节仅用于识别待迁移删除的数据结构，不应继续作为主流程实现依据。
...
说明：候选配置和己方配置中的个体资质字段均保存界面显示值；PVP 面板计算内部使用 `有效个体资质 = 显示个体资质 × 6`。不要把转换后的有效值写回配置字段。
